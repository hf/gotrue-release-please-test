...
func (p *MagicLinkParams) Validate() error {
	if p.Email == "" {
		return unprocessableEntityError("validation_failed", "Password recovery requires an email")
	}
	var err error
...
func (a *API) MagicLink(w http.ResponseWriter, r *http.Request) error {
	...
	if err != nil {
		return badRequestError("bad_json", "Could not read verification params: %v", err).WithInternalError(err)
	}
