...
func (a *API) loadFlowState(w http.ResponseWriter, r *http.Request) (context.Context, error) {
	...
	if state == "" {
		return nil, badRequestError("bad_oauth_callback", "OAuth state parameter missing")
	}
...
func (a *API) oAuthCallback(ctx context.Context, r *http.Request, providerType string) (*OAuthProviderData, error) {
	...
	if oauthCode == "" {
		return nil, badRequestError("bad_oauth_callback", "OAuth callback with missing authorization code missing")
	}

	oAuthProvider, err := a.OAuthProvider(ctx, providerType)
	if err != nil {
		return nil, badRequestError("oauth_provider_not_supported", "Unsupported provider: %+v", err).WithInternalError(err)
	}
