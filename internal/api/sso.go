...
func (p *SingleSignOnParams) validate() (bool, error) {
	...
	if hasProviderID && hasDomain {
		return hasProviderID, badRequestError("validation_failed", "Only one of provider_id or domain supported")
	} else if !hasProviderID && !hasDomain {
		return hasProviderID, badRequestError("validation_failed", "A provider_id or domain needs to be provided")
	}
...
func (a *API) SingleSignOn(w http.ResponseWriter, r *http.Request) error {
	...
	if err := json.Unmarshal(body, &params); err != nil {
		return badRequestError("bad_json", "Unable to parse request body as JSON").WithInternalError(err)
	}
	...
	if flowType == models.PKCEFlow {
		...
		flowState := models.NewFlowState(models.SSOSAML.String(), codeChallenge, codeChallengeMethodType, models.SSOSAML)
	...
	if hasProviderID {
		...
		if models.IsNotFoundError(err) {
			return notFoundError("sso_provider_not_found", "No such SSO provider")
		} else if err != nil {
		...
		if models.IsNotFoundError(err) {
			return notFoundError("sso_provider_not_found", "No SSO provider assigned for this domain")
		} else if err != nil {
