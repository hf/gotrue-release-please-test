...
func (a *API) EnrollFactor(w http.ResponseWriter, r *http.Request) error {
	...
	if err := json.Unmarshal(body, params); err != nil {
		return badRequestError("bad_json", "invalid body: unable to parse JSON").WithInternalError(err)
	}

	if params.FactorType != models.TOTP {
		return badRequestError("validation_failed", "factor_type needs to be totp")
	}
	...
	if len(factors) >= int(config.MFA.MaxEnrolledFactors) {
		return unprocessableEntityError("too_many_enrolled_mfa_factors", "Enrolled factors exceed allowed limit, unenroll to continue")
	}
	...
	if numVerifiedFactors >= config.MFA.MaxVerifiedFactors {
		return forbiddenError("too_many_enrolled_mfa_factors", "Maximum number of enrolled factors reached, unenroll to continue")
	}
	...
	err = a.db.Transaction(func(tx *storage.Connection) error {
		if terr := tx.Create(factor); terr != nil {
			...
			if pgErr.IsUniqueConstraintViolated() {
				return unprocessableEntityError("mfa_factor_name_conflict", fmt.Sprintf("A factor with the friendly name %q for this user likely already exists", factor.FriendlyName))
			}
			return terr
...
func (a *API) VerifyFactor(w http.ResponseWriter, r *http.Request) error {
	...
	if err != nil {
		if models.IsNotFoundError(err) {
			return notFoundError("mfa_factor_not_found", "MFA factor with the provided challenge ID not found")
		}
	...
	if challenge.VerifiedAt != nil || challenge.IPAddress != currentIP {
		return unprocessableEntityError("mfa_ip_address_mismatch", "Challenge and verify IP addresses mismatch")
	}
	...
	if challenge.HasExpired(config.MFA.ChallengeExpiryDuration) {
		...
		return unprocessableEntityError("mfa_challenge_expired", "MFA challenge %v has expired, verify against another challenge or create a new challenge.", challenge.ID)
	}
	...
	if config.Hook.MFAVerificationAttempt.Enabled {
		...
		if output.Decision == hooks.HookRejection {
			...
			return forbiddenError("mfa_verification_rejected", output.Message)
		}
	}
	if !valid {
		return unprocessableEntityError("mfa_verification_failed", "Invalid TOTP code entered")
	}
...
func (a *API) UnenrollFactor(w http.ResponseWriter, r *http.Request) error {
	...
	if factor.IsVerified() && !session.IsAAL2() {
		return unprocessableEntityError("insufficient_aal", "AAL2 required to unenroll verified factor")
	}
	if !factor.IsOwnedBy(user) {
		return internalServerError(InvalidFactorOwnerErrorMessage)
	}
