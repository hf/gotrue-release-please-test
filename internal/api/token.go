...
func (a *API) ResourceOwnerPasswordGrant(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	...
	if err := json.Unmarshal(body, params); err != nil {
		return badRequestError("bad_json", "Could not read password grant params: %v", err)
	}
	...
	if params.Email != "" && params.Phone != "" {
		return badRequestError("validation_failed", "Only an email address or phone number should be provided on login.")
	}
	...
	if params.Email != "" {
		...
		if !config.External.Email.Enabled {
			return unprocessableEntityError("email_provider_disabled", "Email logins are disabled")
		}
		user, err = models.FindUserByEmailAndAudience(db, params.Email, aud)
	} else if params.Phone != "" {
		provider = "phone"
		if !config.External.Phone.Enabled {
			return unprocessableEntityError("phone_provider_disabled", "Phone logins are disabled")
		}
...
func (a *API) PKCE(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	...
	if params.AuthCode == "" || params.CodeVerifier == "" {
		return badRequestError("validation_failed", "invalid request: both auth code and code verifier should be non-empty")
	}
	...
	if models.IsNotFoundError(err) || flowState.UserID == nil {
		return notFoundError("flow_state_not_found", "invalid flow state, no valid flow state found")
	} else if err != nil {
		return err
	}
	if flowState.IsExpired(a.config.External.FlowStateExpiryDuration) {
		return unprocessableEntityError("flow_state_expired", "invalid flow state, flow state has expired")
	}
	...
	if err := flowState.VerifyPKCE(params.CodeVerifier); err != nil {
		return forbiddenError("CHECK", err.Error())
	}
