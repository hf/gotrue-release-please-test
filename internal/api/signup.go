...
func (a *API) validateSignupParams(ctx context.Context, p *SignupParams) error {
	...
	if p.Password == "" {
		return badRequestError("validation_failed", "Signup requires a valid password")
	}
	...
	if p.Email != "" && p.Phone != "" {
		return badRequestError("validation_failed", "Only an email address or phone number should be provided on signup.")
	}
	...
	if p.Phone != "" && p.CodeChallenge != "" {
		return badRequestError("validation_failed", "PKCE not supported for phone signups")
	}
...
func (a *API) Signup(w http.ResponseWriter, r *http.Request) error {
	...
	if config.DisableSignup {
		return forbiddenError("signup_disabled", "Signups not allowed for this instance")
	}
	...
	if err := json.Unmarshal(body, params); err != nil {
		return badRequestError("bad_json", "Could not read Signup params: %v", err).WithInternalError(err)
	}
	...
	case "email":
		if !config.External.Email.Enabled {
			return badRequestError("email_provider_disabled", "Email signups are disabled")
		}
	...
	case "phone":
		if !config.External.Phone.Enabled {
			return badRequestError("phone_provider_disabled", "Phone signups are disabled")
		}
	...
	default:
		...
		return badRequestError("validation_failed", msg)
	}
		...
		if params.Provider == "email" && !user.IsConfirmed() {
			if config.Mailer.Autoconfirm {
				...
				if terr = sendConfirmation(tx, user, mailer, config.SMTP.MaxFrequency, referrer, externalURL, config.Mailer.OtpLength, flowType); terr != nil {
					if errors.Is(terr, MaxFrequencyLimitError) {
						...
						return tooManyRequestsError("over_email_send_rate", fmt.Sprintf("For security purposes, you can only request this after %d seconds.", left))
					}
			...
			if config.Sms.Autoconfirm {
				...
				if _, terr := a.sendPhoneConfirmation(ctx, tx, user, params.Phone, phoneConfirmationOtp, smsProvider, params.Channel); terr != nil {
					return unprocessableEntityError("sms_send_failed", "Error sending confirmation sms: %v", terr).WithInternalError(terr)
				}
			}
		}
	...
	if err != nil {
		reason := "over_email_send_rate"
		if params.Provider == "phone" {
			reason = "over_sms_send_rate"
		}
		...
		if errors.Is(err, MaxFrequencyLimitError) {
			...
			if config.Mailer.Autoconfirm || config.Sms.Autoconfirm {
				return unprocessableEntityError("user_already_exists", "User already registered")
			}
