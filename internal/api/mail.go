...
func (a *API) adminGenerateLink(w http.ResponseWriter, r *http.Request) error {
	...
	if err := json.Unmarshal(body, params); err != nil {
		return badRequestError("bad_json", "Could not parse JSON: %v", err).WithInternalError(err)
	}
	...
	if err != nil {
		if models.IsNotFoundError(err) {
			...
			default:
				return notFoundError("user_not_found", "User with this email not found")
			}
		} else {
	...
	err = db.Transaction(func(tx *storage.Connection) error {
		...
		case "email_change_current", "email_change_new":
			if !config.Mailer.SecureEmailChangeEnabled && params.Type == "email_change_current" {
				return badRequestError("validation_failed", "Enable secure email change to generate link for current email")
			}
		...
		default:
			return badRequestError("validation_failed", "Invalid email action link type requested: %v", params.Type)
		}
...
func validateEmail(email string) (string, error) {
	if email == "" {
		return "", badRequestError("validation_failed", "An email address is required")
	}
	if err := checkmail.ValidateFormat(email); err != nil {
		return "", badRequestError("validation_failed", "Unable to validate email address: "+err.Error())
	}
