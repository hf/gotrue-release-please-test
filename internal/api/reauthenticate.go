...
func (a *API) Reauthenticate(w http.ResponseWriter, r *http.Request) error {
	...
	if email == "" && phone == "" {
		return badRequestError("validation_failed", "Reauthentication requires the user to have an email or a phone number")
	}

	if email != "" {
		if !user.IsConfirmed() {
			return unprocessableEntityError("email_not_confirmed", "Please verify your email first.")
		}
	} else if phone != "" {
		if !user.IsPhoneConfirmed() {
			return unprocessableEntityError("phone_not_confirmed", "Please verify your phone first.")
		}
	}
...
func (a *API) verifyReauthentication(nonce string, tx *storage.Connection, config *conf.GlobalConfiguration, user *models.User) error {
	if user.ReauthenticationToken == "" || user.ReauthenticationSentAt == nil {
		return unprocessableEntityError("reauthentication_not_valid", InvalidNonceMessage)
	}
	var isValid bool
	if user.GetEmail() != "" {
		...
		if config.Sms.IsTwilioVerifyProvider() {
			...
			if err := smsProvider.(*sms_provider.TwilioVerifyProvider).VerifyOTP(string(user.Phone), nonce); err != nil {
				return expiredTokenError("CHECK", "Token has expired or is invalid").WithInternalError(err)
			}
			return nil
		} else {
		...
		return unprocessableEntityError("CHECK", "Reauthentication requires an email or a phone number")
	}
	if !isValid {
		return badRequestError("CHECK", InvalidNonceMessage)
	}
