...
func (p *ResendConfirmationParams) Validate(config *conf.GlobalConfiguration) error {
	...
	default:
		...
		return badRequestError("validation_failed", "Missing one of these types: signup, email_change, sms, phone_change")

	}
	if p.Email == "" && p.Type == signupVerification {
		return badRequestError("validation_failed", "Type provided requires an email address")
	}
	if p.Phone == "" && p.Type == smsVerification {
		return badRequestError("validation_failed", "Type provided requires a phone number")
	}
	...
	if p.Email != "" && p.Phone != "" {
		...
		if !config.External.Email.Enabled {
			return badRequestError("email_provider_disabled", "Email logins are disabled")
		}
		...
		if !config.External.Phone.Enabled {
			return badRequestError("phone_provider_disabled", "Phone logins are disabled")
		}
		...
		return badRequestError("validation_failed", "Missing email address or phone number")
	}
	return nil
}
