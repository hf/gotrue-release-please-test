...
func (a *API) loadSSOProvider(w http.ResponseWriter, r *http.Request) (context.Context, error) {
	...
	if err != nil {
		...
		return nil, notFoundError("sso_provider_not_found", "SSO Identity Provider not found")
	}
...
func (p *CreateSSOProviderParams) validate(forUpdate bool) error {
	if !forUpdate && p.Type != "saml" {
		return badRequestError("validation_failed", "Only 'saml' supported for SSO provider type")
	} else if p.MetadataURL != "" && p.MetadataXML != "" {
		return badRequestError("validation_failed", "Only one of metadata_xml or metadata_url needs to be set")
	} else if !forUpdate && p.MetadataURL == "" && p.MetadataXML == "" {
		return badRequestError("validation_failed", "Either metadata_xml or metadata_url must be set")
	} else if p.MetadataURL != "" {
		metadataURL, err := url.ParseRequestURI(p.MetadataURL)
		if err != nil {
			return badRequestError("validation_failed", "metadata_url is not a valid URL")
		}

		if metadataURL.Scheme != "https" {
			return badRequestError("validation_failed", "metadata_url is not a HTTPS URL")
		}
	}
...
func parseSAMLMetadata(rawMetadata []byte) (*saml.EntityDescriptor, error) {
	if !utf8.Valid(rawMetadata) {
		return nil, badRequestError("validation_failed", "SAML Metadata XML contains invalid UTF-8 characters, which are not supported at this time")
	}
	...
	if metadata.EntityID == "" {
		return nil, badRequestError("validation_failed", "SAML Metadata does not contain an EntityID")
	}

	if len(metadata.IDPSSODescriptors) < 1 {
		return nil, badRequestError("validation_failed", "SAML Metadata does not contain any IDPSSODescriptor")
	}

	if len(metadata.IDPSSODescriptors) > 1 {
		return nil, badRequestError("validation_failed", "SAML Metadata contains multiple IDPSSODescriptors")
	}
...
func fetchSAMLMetadata(ctx context.Context, url string) ([]byte, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, badRequestError("saml_metadata_fetch_failed", "HTTP %v error fetching SAML Metadata from URL '%s'", resp.StatusCode, url)
	}
...
func (a *API) adminSSOProvidersCreate(w http.ResponseWriter, r *http.Request) error {
	...
	if err := json.Unmarshal(body, &params); err != nil {
		return badRequestError("bad_json", "Unable to parse JSON").WithInternalError(err)
	}
	...
	if existingProvider != nil {
		return badRequestError("saml_ipd_already_exists", "SAML Identity Provider with this EntityID (%s) already exists", metadata.EntityID)
	}
	...
	for _, domain := range params.Domains {
		...
		if existingProvider != nil {
			return badRequestError("sso_domain_already_exists", "SSO Domain '%s' is already assigned to an SSO identity provider (%s)", domain, existingProvider.ID.String())
		}
...
func (a *API) adminSSOProvidersUpdate(w http.ResponseWriter, r *http.Request) error {
	...
	if params.MetadataXML != "" || params.MetadataURL != "" {
		...
		if provider.SAMLProvider.EntityID != metadata.EntityID {
			return badRequestError("saml_entity_id_mismatch", "SAML Metadata can be updated only if the EntityID matches for the provider; expected '%s' but got '%s'", provider.SAMLProvider.EntityID, metadata.EntityID)
		}
	...
	for _, domain := range params.Domains {
		...
		if existingProvider != nil {
			if existingProvider.ID == provider.ID {
				...
				return badRequestError("sso_domain_already_exists", "SSO domain '%s' already assigned to another provider (%s)", domain, existingProvider.ID.String())
			}
		} else {
			modified = true
	...
	if modified {
		if err := db.Transaction(func(tx *storage.Connection) error {
			...
			return unprocessableEntityError("conflict", "Updating SSO provider failed, likely due to a conflict. Try again?").WithInternalError(err)
		}
	}
