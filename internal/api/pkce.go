...
func isValidCodeChallenge(codeChallenge string) (bool, error) {
	...
	case codeChallengeLength < MinCodeChallengeLength, codeChallengeLength > MaxCodeChallengeLength:
		return false, badRequestError("validation_failed", "code challenge has to be between %v and %v characters", MinCodeChallengeLength, MaxCodeChallengeLength)
	case !codeChallengePattern.MatchString(codeChallenge):
		return false, badRequestError("validation_failed", "code challenge can only contain alphanumeric characters, hyphens, periods, underscores and tildes")
	default:
		return true, nil
	}
}
...
func issueAuthCode(tx *storage.Connection, user *models.User, expiryDuration time.Duration, authenticationMethod models.AuthenticationMethod) (string, error) {
	...
	if err != nil && models.IsNotFoundError(err) {
		return "", unprocessableEntityError("flow_state_not_found", "No valid flow state found for user.")
	} else if err != nil {
		return "", err
	}
...
func validatePKCEParams(codeChallengeMethod, codeChallenge string) error {
	...
	case (codeChallenge == "") != (codeChallengeMethod == ""):
		return badRequestError("validation_failed", InvalidPKCEParamsErrorMessage)
	case codeChallenge != "":
