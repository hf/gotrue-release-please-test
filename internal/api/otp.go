...
func (p *OtpParams) Validate() error {
	if p.Email != "" && p.Phone != "" {
		return badRequestError("validation_failed", "Only an email address or phone number should be provided")
	}
	if p.Email != "" && p.Channel != "" {
		return badRequestError("validation_failed", "Channel should only be specified with Phone OTP")
	}
...
func (a *API) Otp(w http.ResponseWriter, r *http.Request) error {
	...
	if err = json.Unmarshal(body, params); err != nil {
		return badRequestError("bad_json", "Could not read verification params: %v", err)
	}
	...
	if ok, err := a.shouldCreateUser(r, params); !ok {
		return badRequestError("signup_disabled", "Signups not allowed for otp")
	} else if err != nil {
		return err
	}
	...
	return badRequestError("validation_failed", "One of email or phone must be set")
}
...
func (a *API) SmsOtp(w http.ResponseWriter, r *http.Request) error {
	...
	if !config.External.Phone.Enabled {
		return badRequestError("phone_provider_disabled", "Unsupported phone provider")
	}
	var err error
	...
	if err := json.Unmarshal(body, params); err != nil {
		return badRequestError("bad_json", "Could not read sms otp params: %v", err)
	}
	...
	err = db.Transaction(func(tx *storage.Connection) error {
		...
		if terr != nil {
			return internalServerError("Error finding SMS provider").WithInternalError(err)
		}
		mID, serr := a.sendPhoneConfirmation(ctx, tx, user, params.Phone, phoneConfirmationOtp, smsProvider, params.Channel)
		if serr != nil {
			return badRequestError("sms_send_failed", "Error sending sms OTP: %v", serr).WithInternalError(serr)
		}
		messageID = mID
		return nil
	})
